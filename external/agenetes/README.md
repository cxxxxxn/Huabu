...
ThreadStore、EventLogStore、TurnStore 可以返回值或 Promise，Agenetes 会等待持久化完成；调用方须等待上述生命周期与持久化读取方法，`get` 仍是活跃 handle 查找，`tail` 与 `notifications` 仍是异步可迭代对象。事件与状态通知只在写入成功后发布，每个 thread 的状态上报按序排队、`close` 会等待队列排空，写入失败会在随后的 `record`、`run` 或 `close` 上暴露。这些保证仅限进程内，不构成跨存储的分布式事务。
