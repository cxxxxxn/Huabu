...
| Instance            | `Agenetes.updateAnnotations(namespace, threadId, patch) -> ThreadRecord`        | —                                                               | Synchronously merge host-owned annotation keys without realizing a workload.                                   |
...
Each `ThreadRecord` may additionally contain `annotations?: Record<string, unknown>` for host-owned durable metadata. Annotations are separate from `spec` and `state`: drivers neither own them nor receive them in recovery/fork inputs. `Agenetes.updateAnnotations(namespace, threadId, patch): ThreadRecord` synchronously reads the current record, shallow-merges annotation keys, persists the result, and returns it without realizing a workload or changing its spec/state. Supplied keys replace whole values; omitted keys survive; `null` is an ordinary stored value, not deletion. Values must be JSON-compatible (null, strings, booleans, finite numbers, arrays, and plain objects); invalid patches throw `AgenetesError` with code `invalid_annotations` before any write. A missing record throws `thread_not_found` with namespace/threadId details and is never created implicitly. Annotation values are copied on update and query so nested host mutations do not become unpersisted annotation writes.

The instance remains the sole `ThreadStore` writer: annotation updates and driver up-reports each perform a synchronous read/merge/write with no asynchronous gap, so no async mutex is needed. This is a single-instance/single-writer guarantee, not cross-process concurrency control. Driver up-reports replace only the state snapshot while preserving the latest annotations, and annotation updates do not publish on the driver-metadata `notifications()` stream. Initial records are persisted before subscribing to `onState`, so a driver's synchronous initial report is not overwritten by realization. Annotation changes survive record validation, listing, repeated creation, restart/resume, and rehome (including determinate rollback). Fork **deep-copies** the source annotations into the fresh target, because host metadata describes the conversation being branched; subsequent source/target updates are independent. Target driver state still starts fresh, and the framework applies no host-specific metadata policy.

`FileThreadStore` preserves and validates optional annotations on every read; malformed annotation envelopes fail with `invalid_persisted_record`. Existing `agenetes-v2` records without annotations remain valid and retain the absent field, so no version bump or migration is needed. The store's `upsert` remains a whole-record replacement primitive; host applications must use the instance mutation API rather than writing around the sole-writer boundary.
