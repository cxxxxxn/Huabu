...
| Instance            | `Agenetes.updateHostMetadata(namespace, threadId, patch) -> Promise<ThreadRecord>` | —                                                               | Merge host-owned metadata keys without realizing a workload.                                                   |
...
## Asynchronous persistence / 异步持久化

ThreadStore, EventLogStore, and TurnStore implementations may answer with a value or a promise, and Agenetes awaits them. Callers therefore await `create`, `fork`, `rehome`, `close`, `record`, `records`, `updateHostMetadata`, `history`, `historyPage`, and `logMetadata`; `get` remains a live handle lookup, and `tail` and `notifications` remain async iterables. The file and in-memory stores keep their synchronous implementations — the port admits a promise, it does not require one.

A write completes before the event or state notification that announces it. Per-thread state reports are queued in report order, `close` drains that queue, and a failed state write surfaces on the next `record`, `run`, or `close` rather than silently. A queue belongs to one thread and one namespace, so `records(namespace)` waits only for the writes that can change its own answer and a parked write in another namespace never holds it open. Rehome serializes its writes and its compensation the same way. Hosts still coordinate running turns and namespace changes themselves: this is process-local ordering, not a distributed transaction across stores.

`historyPage` reads the fold boundary, the live tail, and the persisted page separately, so a turn that folds between those reads would otherwise be returned twice — once as a persisted turn and once as the active tail. The boundary is therefore re-read after paging and the page rebuilt against the settled layout; a boundary that keeps moving answers from the persisted page alone, one turn behind rather than self-contradictory.

ThreadStore、EventLogStore、TurnStore 可以返回值或 Promise，Agenetes 会等待持久化完成；调用方须等待上述生命周期与持久化读取方法，`get` 仍是活跃 handle 查找，`tail` 与 `notifications` 仍是异步可迭代对象。事件与状态通知只在写入成功后发布，每个 thread 的状态上报按序排队、`close` 会等待队列排空，写入失败会在随后的 `record`、`run` 或 `close` 上暴露。队列同时归属一个 thread 与一个 namespace，因此 `records(namespace)` 只等待可能改变自身答案的写入，另一个 namespace 中挂起的写入不会把它一直堵住。`historyPage` 分别读取折叠边界、live tail 与持久页，若某轮在这些读取之间折叠，同一 turn 会既作为持久 turn、又作为 active tail 返回两次；因此分页后会重新读取边界并按稳定后的布局重组，边界持续移动时则只返回持久页，宁可落后一轮也不自相矛盾。这些保证仅限进程内，不构成跨存储的分布式事务。
...
Each `ThreadRecord` may additionally contain `hostMetadata?: Record<string, unknown>` for host-owned durable metadata. Host metadata is separate from `spec` and `state`: drivers neither own it nor receive it in recovery/fork inputs. `Agenetes.updateHostMetadata(namespace, threadId, patch): ThreadRecord` synchronously reads the current record, shallow-merges host metadata keys, persists the result, and returns it without realizing a workload or changing its spec/state. Supplied keys replace whole values; omitted keys survive; `null` is an ordinary stored value, not deletion. Values must be JSON-compatible (null, strings, booleans, finite numbers, arrays, and plain objects); invalid patches throw `AgenetesError` with code `invalid_host_metadata` before any write. A missing record throws `thread_not_found` with namespace/threadId details and is never created implicitly. Host metadata values are copied on update and query so nested host mutations do not become unpersisted host metadata writes.

The instance remains the sole `ThreadStore` writer: host metadata updates and driver up-reports each perform a synchronous read/merge/write with no asynchronous gap, so no async mutex is needed. This is a single-instance/single-writer guarantee, not cross-process concurrency control. Driver up-reports replace only the state snapshot while preserving the latest host metadata, and host metadata updates do not publish on the driver-metadata `notifications()` stream. Initial records are persisted before subscribing to `onState`, so a driver's synchronous initial report is not overwritten by realization. Host metadata changes survive record validation, listing, repeated creation, restart/resume, and rehome (including determinate rollback). Fork **deep-copies** the source host metadata into the fresh target, because host metadata describes the conversation being branched; subsequent source/target updates are independent. Target driver state still starts fresh, and the framework applies no host-specific metadata policy.

`FileThreadStore` preserves and validates optional host metadata on every read; malformed host metadata envelopes fail with `invalid_persisted_record`. Existing `agenetes-v2` records without host metadata remain valid and retain the absent field, so no version bump or migration is needed. The store's `upsert` remains a whole-record replacement primitive; host applications must use the instance mutation API rather than writing around the sole-writer boundary.
...
`notifications(threadId, namespace?)` optionally restricts delivery to `namespace.name`, matching the namespace's logical identity rather than its storage configuration. Every report reaches both the matching scoped stream and the legacy global per-thread stream. Closing a live handle ends those streams (including silent handles), not scoped subscribers for another namespace with the same historical thread ID. Scoped and legacy channels use collision-free tuple keys; queued snapshots remain ordered and drain before closure.
...
external/agenetes/packages/
  ...
  @agenetes/agent-profile  [CONTROL · durable]       deps: protocol, runtime, zod
  @agenetes/agentlet-gateway [TRANSPORT · stateless] deps: @agentlet/protocol, ws
  @agenetes/agentlet-host  [TRANSPORT · ACP-private] deps: protocol, agent-profile, agentlet-gateway, fastify
