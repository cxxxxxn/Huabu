...
The Lasso HUD uses the Canvas `z-999` interaction layer: it sits above selection outlines, nodes, edges, and node media, and its later mount order keeps it above same-layer multi-selection resize chrome. It remains strictly below body-level `z-index: 1000` Canvas floating toolbars and below dialogs, tooltips, and toasts. Grounding capture continues to exclude it through `data-canvas-grounding-exclude`.
