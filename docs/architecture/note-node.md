...
When a later agent rewrite restores a modified block to its original user-owned baseline, its pending `modified` provenance record is removed. Equality uses the shared normalized Markdown block fingerprint, ignoring duplicate occurrence suffixes; other pending records and operation history are unchanged.
