...
Primary activation validates the row against live Canvas nodes, expands collapsed ancestors, replaces Canvas selection, and reveals the target only while both the React Flow instance and Canvas wrapper are mounted. Preview-capable nodes then open or activate their permanent semantic Preview target in every layout, including Preview fullscreen where Canvas is unmounted. A node already active in a visible Preview group is not reopened, preventing repeated Note editor-focus requests. Frame and legacy Group rows toggle their own disclosure on repeated primary activation and reveal direct children but never invoke Preview; activating an empty Frame emits an accessible status toast. Existing-thread Questions reuse the passive Preview opening contract without creating a thread, focusing the composer, sending input, or realizing Agent work; idle Questions and unsupported node types remain Canvas-only.
