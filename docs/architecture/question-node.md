...
Question naming prefers manual node labels, then Huabu-generated first-prompt titles, then valid ACP `session_info_update.title` fallback, then the local first-user-prompt fallback. Title synchronization uses the server Canvas executor with `labelSource: 'agent'` and records the actual committed `{ nodeId, label }` as `lastSyncedNodeLabel` in the thread's title annotation. Under the same Canvas mutex, an atomic guard checks the Question/thread binding, current preferred candidate, and label ownership: user labels and unrelated agent-authored labels cannot be replaced, while exact title-owned ACP labels can be upgraded by Huabu generation. The existing `generate_label` preprocessing profile remains eligible for this provenance-verified case and shares naming work with initialization; failures or no usable model permit a later turn/initialize to retry. Late ACP cannot displace a generated title. ACP validation checks only nonempty strings and a maximum of 120 characters after whitespace normalization; there is no prompt-text filtering, fixed-prefix repair, or migration of old ACP annotations. Existing agent labels without provenance are not assumed to be title-owned. Panel manual titles stay separate. These writes change only label metadata, never the first prompt, status, or transcript. The Canvas node, its Preview tab, and its chat header all read the same node label; existing visual overflow handling is unchanged. Panel-only conversations use the same provider but persist names on the thread instead of creating hidden nodes; see [Preview Workspace](./preview-workspace.md#2-persisted-model).

---

## 3. Node lifecycle

Saving a panel Chat as a Question carries `conversationTitleSource` through canonical node creation into persisted structural metadata (`user | acp | generated | fallback`; a missing cached title is `fallback`). This is the initial conversion source, not current label ownership. Manual titles become `labelSource: 'user'`; all nonmanual copied titles become `auto`, so the existing preprocessing/initialization path can adopt the thread's generated candidate or retry generation from its first prompt. Converted auto labels are excluded from pre-thread semantic-label reuse: neither ACP text nor a deduplicated first-prompt fallback is guessed to be generated. Successful title synchronization records the actual committed label as usual; copied generated titles reuse the durable thread candidate without another provider call. Conversion metadata never grants permission to overwrite user or unrelated agent labels. Older converted Questions with `labelSource: 'agent'` but no exact title-sync provenance remain protected, even if their text matches ACP; there is no unsafe migration or automatic upgrade for those legacy labels.
