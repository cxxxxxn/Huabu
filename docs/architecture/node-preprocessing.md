...
- Label policy: Enrich may propose a label, but Project includes it only when the request label is not a non-empty `user`/`agent` label. Because a request can already be in flight when the user renames a node, the web client re-reads the latest node before applying the response and drops its automatic label patch when that current label is user/agent-owned; other response fields still apply. The planner skips `generate_label` for protected labels, including Questions. Chat-to-Question conversion copies the current title once with `labelSource: 'user'` for a manual title or `labelSource: 'agent'` for a nonmanual title, protecting either from ordinary preprocessing. Without a current title, conversion retains the existing node-creation fallback and normal preprocessing eligibility.
- The server enforces ordinary automatic-label protection again at commit: the Canvas executor removes only automatic `label`/`labelSource` fields over a currently protected label, and the content PUT resolves automatic labels against the latest sidecar under the shared Canvas mutex. Question Enrich uses the ordinary `ProviderManager` path independently of panel title generation; it neither publishes generated labels to the conversation-title service nor reuses panel titles. Late Chat generation and ACP updates never rename a Question. Panel title generation retains its own later-turn/initialize retries after undefined output or provider failure; see [panel conversation titles](./agent-architecture.md#panel-conversation-titles).
...
- Undo/redo does not use preprocessing to recreate a sidecar. The existing content queue restores the captured body only after structure-save acknowledgement; preprocessing scheduling, delayed sends, and keepalive sends reject nodes held by that restore barrier. History restores reuse preprocessing cancellation to discard pending timers and advance a projection generation: already-issued responses cannot patch a restored snapshot even after its structure/content saves finish. Issued requests remain completion-tracked so a subsequent DELETE waits before unlinking; cancellation does not abort server work. This fence also covers live-node undo/redo, while newly scheduled preprocessing keeps the existing label-ownership and last-response-wins rules. Delayed sends still require a live node in the captured Canvas, preserving preview exclusions and missing-content guards without another persistence pipeline.
...
- Per-node debounce exists (`preprocessQueue`), but there is no general requestId-based stale-result guard for out-of-order POST responses within one projection generation. History restore and canvas-switch cancellation invalidate older client projections; they do not cancel server persistence. The latest user/agent label is protected at projection time, while summaries, keywords, and other derived fields otherwise remain last-response-wins.
