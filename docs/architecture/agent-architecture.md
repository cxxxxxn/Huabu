...
Chat-to-Question conversion copies the current title once through the existing conversion helper and canonical node creation. A manual title (`source: 'user'`) becomes `labelSource: 'user'`; any nonmanual copied title becomes `labelSource: 'agent'`, protecting it from ordinary `generate_label` preprocessing. Without a current title, conversion preserves the existing node-creation fallback rather than inventing a title. Thereafter the Question's canonical node label alone supplies its Canvas, Preview tab, and chat header names; later Chat title generation, ACP updates, and panel renames never rename the Question. See [Question conversion](./question-node.md#3-node-lifecycle).

[ConversationTitleService](../../apps/server/src/modules/agent/conversation-title.service.ts) owns panel-only names independently of driver session metadata and Question labels. `ThreadRecord.hostMetadata.huabuConversationTitle` persists only one current `{ title, source }` through `Agenetes.updateHostMetadata`; both Disk and SQLite preserve it across restart, driver snapshots, and rehome. Source priority is `user > generated > acp > fallback`: lower-priority updates are rejected rather than saved, and manual naming retains no hidden automatic title. When no higher-priority host title exists, reads may use the driver's current valid ACP title; the last valid ACP title is persisted only while the current host title has source `acp`, not as a separate fallback candidate. The zod-free [shared title utility](../../packages/shared/src/utils/conversation-title.ts), exported as `@huabu/shared/conversation-title`, supplies `normalizeConversationTitle` for host/manual whitespace normalization and 120-character truncation, and `normalizeAcpConversationTitle` for generic ACP validation: non-string, blank, or normalized values longer than 120 characters are rejected, with no prompt-text or fixed-prefix filtering. Invalid updates leave the current host title unchanged; invalid driver metadata can fall back to a valid persisted current ACP title. There is no migration for the old unmerged candidate-field shape. Driver `sessionInfo.title` and ACP prompt assembly are never rewritten.

The built-in Deployment and ACP message adapters initialize the first prompt fallback after durable creation only for standalone Chat. Trusted, resolved Question ownership is passed internally as a `questionOwned` boolean; Question-owned conversations retain their original `generate_label` path rather than initializing panel titles. Initialization attempts asynchronous Huabu generation even when ACP already supplies a title, using only the first persisted user prompt (or the first submission before history is available). `ProviderManager.generateContentMeta` uses canonical `contentMeta` role routing: explicit utility model, otherwise the cheapest eligible model in the global Chat provider, ultimately the global Chat model; it does not require a separate utility configuration. No usable model, undefined output, and provider errors retain ACP or the first-user fallback. They do not persist a terminal attempt marker: a later turn/initialize may retry after model configuration or provider recovery, without a retry loop in one request. Successful generated titles and manual panel titles suppress further generation. Concurrent initialization calls for the same Canvas/thread share one in-flight attempt. Completion checks the current source and rejects generation after a manual rename instead of retaining it; accepted writes preserve unrelated host metadata. Reads never create a handle, spawn ACP, generate a title, or repair stored metadata; legacy threads can derive a read-only fallback from their first persisted user prompt.

Canonical external realization installs a persist-then-notify title metadata subscriber before session bootstrap only when there is no Question owner, including control-first realization. Valid ACP updates can update the panel title only while source priority permits; late ACP cannot overwrite a generated or manual title. The title service neither reads nor writes Canvas nodes. Question preprocessing remains independent: it does not publish labels back to the title service or reuse node labels as panel titles, and ordinary automatic preprocessing always protects non-empty user/agent labels.

The [shared title contracts](../../packages/shared/src/types/api/conversation-title.ts) define `ConversationTitle { title, source }`, where source is `user | acp | generated | fallback | null`. `POST /api/agent/threads/titles/query` accepts `{ canvasId, threadIds }` (at most 100 IDs) and returns `{ titles: Record<string, ConversationTitle> }`, including `{ title: null, source: null }` for an absent durable thread. `PUT /api/agent/threads/:threadId/title?canvasId=...` accepts a trimmed non-empty `{ title }` of at most 120 characters, returns the effective title, and returns `404 thread_not_found` before durable creation. Clients keep pre-send manual names locally until that thread exists. Both endpoints are Canvas-scoped and validate all inputs. No new history surface or title SSE event is introduced: clients query mounted panel threads and refresh after relevant thread events; asynchronous generation may require a bounded follow-up query after the run ends.
