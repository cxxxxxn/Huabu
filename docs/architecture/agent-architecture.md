...
Chat-to-Question conversion persists the initial `conversationTitleSource` on the new node and copies nonmanual labels as `auto`, not `agent`. Initialization excludes these copied labels from pre-thread semantic auto-label reuse, then adopts the durable generated candidate or generates from the first user prompt through the shared in-flight path. This avoids both ACP-as-generated misclassification and lost upgrade ownership without extending the agent-label overwrite exception. See [Question conversion provenance](./question-node.md#3-node-lifecycle); legacy agent labels without exact provenance are not migrated.

[ConversationTitleService](../../apps/server/src/modules/agent/conversation-title.service.ts) owns panel-only names independently of driver session metadata and Question labels. The effective order is manual panel title → Huabu-generated title → current valid ACP title → last valid saved ACP title → first-user-prompt fallback. The zod-free [shared title utility](../../packages/shared/src/utils/conversation-title.ts), exported as `@huabu/shared/conversation-title`, supplies `normalizeConversationTitle` for host/manual whitespace normalization and 120-character truncation, and `normalizeAcpConversationTitle` for generic ACP validation: non-string, blank, or normalized values longer than 120 characters are rejected, with no prompt-text or fixed-prefix filtering. Invalid updates preserve any valid saved ACP candidate. Candidates live in `ThreadRecord.annotations.huabuConversationTitle`, updated through `Agenetes.updateAnnotations`; both Disk and SQLite preserve them across restart, driver snapshots, and rehome. Existing `acp` annotations remain fallback candidates without migration. Driver `sessionInfo.title` and ACP prompt assembly are never rewritten.

The built-in Deployment and ACP message adapters initialize the first prompt fallback after durable creation. Initialization attempts asynchronous Huabu generation even when ACP already supplies a title, using only the first persisted user prompt (or the first submission before history is available). `ProviderManager.generateContentMeta` uses canonical `contentMeta` role routing: explicit utility model, otherwise the cheapest eligible model in the global Chat provider, ultimately the global Chat model; it does not require a separate utility configuration. No usable model, undefined output, and provider errors retain ACP or the first-user fallback. They do not persist a terminal attempt marker: a later turn/initialize may retry after model configuration or provider recovery, without a retry loop in one request. Successful generated titles and manual panel titles suppress further generation. Concurrent initialization and durable Question Enrich share one in-flight attempt; Questions can also reuse a semantic auto label generated before thread creation. Completion merges into current annotations, preserving manual precedence. Reads never create a handle, spawn ACP, generate a title, or repair stored metadata; legacy threads can derive a read-only fallback from their first persisted user prompt.

Canonical external realization installs a persist-then-notify metadata subscriber before session bootstrap, including control-first realization. Generated titles, or ACP while generation is unavailable, synchronize the owning selectable or fixed Question's `data.label` with `labelSource: 'agent'`. `lastSyncedNodeLabel: { nodeId, label }` in the thread title annotation records the actual committed node label, including name deduplication. The service holds the canonical Canvas mutex across owner resolution, the already-locked Canvas executor, and provenance persistence. Its atomic guard requires the same Question/thread, an unchanged generated-or-ACP candidate, and either an unprotected label or an agent label exactly matching title-sync provenance. User labels and unrelated agent labels are never replaced; legacy agent labels without provenance are not inferred to be title-owned. A generated result can therefore upgrade a title-owned ACP fallback, while late ACP cannot overwrite generation. There is no fixed-prefix recovery. Panel manual titles are not copied onto nodes, and these writes never change Question content or status. Ordinary automatic preprocessing commits retain their user/agent protection; only title-service synchronization performs the provenance-guarded upgrade.
