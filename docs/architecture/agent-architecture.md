...
- **Ink intent reporting is turn-scoped**: the pi driver supports host-provided per-turn tool overlays that are merged into the live Agent for one `run()` and restored in `finally`, without changing the durable workload spec. Built-in `ink-intent` turns overlay `report_ink_intent`; ordinary text turns never see it, and recovered legacy Deployments receive it without a spec migration. The server validates the report, hides its tool row, folds the interpretation into history/search metadata, and gates execution against the currently active Ink turn. An accepted Ink turn consumes a new Question's pending-label marker through the same server-owned terminal lifecycle projection; pre-acceptance failures leave it available for retry.
- **Node-backed invocation is service-owned**: `POST /api/agent`, direct RFS, and Interactive View use `AgentThreadService`. After admission it resolves fresh node identity regardless of `agentBindingPolicy`, rejects a conflicting requested Profile, registers cancellation before slow preparation (including deferred Web/RFS envelope construction), and projects `running` with a fresh `invocationToken` before dispatch. `AgentNodeLifecycle` fills only freshly read empty never-submitted content, using prior token/history evidence, and projects terminal `done`/`error` plus unread attention under the Canvas mutex. Terminal writes check the current token and release admission only after settlement cleanup. Ordinary node-less Chat has no node FSM.
- **Binding is a separate monotonic axis**: `AgentNodeBindingCoordinator` confirms the validated `agenetes.record(namespace, threadId)` and persists `bindingState: bound`. A first external control can bind without a prompt, content write, or invocation token. Internal `runAgent` exposes an awaited `onExecutionCreated` seam after canonical create and before settings controls or `run`; thread-associated Jobs use the same confirmation without changing their existing execution strategy or same-driver spec semantics. A failed Bound write cannot roll back canonical binding; the next guarded operation completes promotion. Missing records on Bound nodes are explicit execution errors, never permission to rebind. No native Session ID or live process is required to establish Bound.
- **Prompt Frame snapshots**: for every Agent Node, the service compiles recognized Space Prompt Frames into a bounded user-authored preamble on first realization and persists it in the workload (`hostContext.spacePrompt` for built-in, a dedicated `initialPreamble` fragment for ACP). Later turns reuse that snapshot, including the intentional absence on realized legacy threads. Node-less Canvas Chat does not collect Prompt Frames.
...
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ask` / `operate` | `POST /api/agent` ([agent.route.ts](../../apps/server/src/modules/agent/agent.route.ts) → [AgentThreadService](../../apps/server/src/modules/agent/agent-thread.service.ts)) | Main chat path; ask is read-only, operate can write. The service dispatches built-in or ACP Deployments and owns node-backed invocation regardless of binding policy. |
...
`space_commands` covers 13 commands ([space-operations.ts](../../packages/shared/src/types/api/space-operations.ts)): `CREATE_NODES`, `DELETE_NODES`, `MERGE_NODE_DATA`, `SET_NODE_PARENT`, `DISSOLVE_FRAME`, `SET_NODE_GEOMETRY`, `REORDER_NODES`, `CONNECT_NODES`, `DISCONNECT_EDGES`, `SET_EDGE_STYLE`, `ALIGN_NODES`, `DISTRIBUTE_NODES`, and `SET_FRAME_LAYOUT` — the Agent subset of [`CanvasCommand`](../../packages/shared/src/types/canvas/command.ts), excluding `SET_NODE_LOCKED`, `SET_NODE_SELECTION`, `CHANGE_NODE_TYPE`, and internal `APPLY_MEASURED_HEIGHT`. Portal Pin commands and their source-to-World routing are removed. The canonical Zod contracts are converted to the JSON Schema shape expected by pi-ai in [zod-tool-schema.ts](../../apps/server/src/modules/agent/tools/zod-tool-schema.ts). Commands execute server-side against the addressed Canvas, persist, and return deltas; see [canvas-command-architecture.md](./canvas-command-architecture.md).
...
- `POST /api/agent` completes canonical rendering and required Ink/model validation before invoking `AgentThreadService`, changing Question lifecycle, or writing SSE headers. A required partial-Ink image that cannot be materialized/inlined and an effective built-in model without image input both return a structured pre-acceptance rejection; external ACP receives the canonical image block without an unsupported capability claim.
...
- **On disk (Chat-V2 two-tier log).** A canvas's namespace `storage.root` is its `.history/` dir (`canvasAcpNamespace(canvasId)`), so Tier 1 remains `.history/chat_v2/<threadId>.events.jsonl`, the append-only `AgentStreamEvent` delta log written by [`FileEventLogStore`](../../external/agenetes/packages/agenetes/src/event-log.ts), while Tier 2 is the Space-owned `.history/chat_v2/turns.sqlite`, whose `turns` rows hold folded `AgentTurn` records and whose indexed ordinal ranges power bounded history pages through [`FileTurnStore`](../../external/agenetes/packages/agenetes/src/turn-store.ts). Existing `.history/chat_v2/<threadId>.turns.jsonl` files are imported lazily and transactionally on first access; malformed or trailing partial lines are skipped consistently with the former reader, successful import retires the source to `.bak`, and a failed import or retirement leaves the source retryable without duplicate rows. These conversation stores deliberately contain the turn submission and assistant/tool transcript only; system prompts, ACP bootstrap text, fixed-node initial instructions, and captured Space Prompt are workload configuration rather than conversation parts. Durable workload records remain in `.history/threads.json` ([`FileThreadStore`](../../external/agenetes/packages/agenetes/src/thread-store.ts)). With `HUABU_DEBUG_PROMPT` enabled, Huabu writes the fully assembled per-turn diagnostic prompt under `huabu.prompt.log/<threadId>.prompt.log`. Legacy chat files may still be folded into per-thread Chat-V2 JSONL during workspace activation by [`migrate-chat-turns.ts`](../../apps/server/src/modules/workspace/migrations/migrate-chat-turns.ts); the live Tier-2 adapter performs the final SQLite import.
- `Agenetes.history()` remains the complete-history surface used by model recovery, replay, fork preparation, and search-oriented consumers. `Agenetes.historyPage()` is a distinct bounded display surface: the store owns opaque exclusive cursors bound to thread and replacement generation, ordinary appends do not invalidate them, and `replace`/`delete` during rehome does. A display group begins at a non-null request and absorbs following null-request resume turns; leading null-request records form one synthetic visible group, and page boundaries never split a group. Only the newest page may request the incomplete Tier-1 tail: a non-null tail consumes one requested group, a null-request tail attaches to the latest persisted group or becomes an orphan group, and the active group keeps the same identity when its new-user tail is folded into Tier 2. Active groups also expose the first Tier-1 turn so the HTTP projection can mark the exact message suffix replaced by reconnect replay without removing committed continuation context.
...
Move validates the whole selection and acquires all turn leases before closing any handle. After destination insertion, each dormant handle is synchronously closed immediately before its rehome, inside the existing compensation boundary. A cached Deployment is not an active invocation. Close releases the runtime, not its durable record or history; driver teardown must succeed before Agenetes detaches persistence listeners and ends notification streams. A thrown close rejects the move without rehoming that thread; the driver may have partially torn down, so no healthy-live-handle guarantee is implied. Unbound drafts without history do not manufacture execution records. Successful moves recover lazily in the destination; determinate compensation restores durable source ownership without eagerly recreating handles.

Move maps execution/binding failures to bounded `MOVE_*` errors, never raw Agent errors. Unknown rehome outcomes and failed compensation retain the destination for reconciliation, including request-created Spaces. Existing realized ACP controls have their own admission behavior; Move does not change control-operation locking policy or claim to serialize every in-flight control RPC.

---

### Panel conversation titles

Panel Chat and Question nodes share one naming policy through [ConversationTitleService](../../apps/server/src/modules/agent/conversation-title.service.ts). Storage follows the current owner: an unbound Chat uses `ThreadRecord.hostMetadata.huabuConversationTitle`, while a Question uses its canonical node `label` plus protected `conversationTitleSource` provenance through [conversation-title-node-store.ts](../../apps/server/src/modules/agent/conversation-title-node-store.ts). Reads and writes resolve Question ownership first; retained thread metadata is not a second title authority. The node label supplies Canvas, Preview tab, chat header, and title-query values without changing prompt content, status, or transcript.
...
Question preprocessing delegates to `initializeQuestion()` instead of independently generating a label. It can establish a fallback and generate from existing Question content before any durable thread exists, without starting ACP. Node-backed send admission calls `ensureFallback()` after lifecycle start and before slow Agent preparation, preserving a name even if preparation fails. Both built-in Deployment and ACP message adapters call `start()` after durable realization, for either Chat or Question ownership: fallback preparation is awaited, utility-model generation is not. Initialization uses the first persisted user prompt when available, otherwise Question content or the current submission, and attempts Huabu generation even when ACP supplies a title. `ProviderManager.generateContentMeta` uses canonical `contentMeta` role routing: explicit utility model, otherwise the cheapest eligible model in the global Chat provider, ultimately the global Chat model. Undefined output, missing models, and provider errors retain the accepted ACP/fallback title and permit a later turn/initialize retry; successful generated titles and protected names suppress generation. Concurrent initialization shares one in-flight attempt per Canvas/thread (or node before association). Completion rechecks current ownership, source, and applicable content freshness under the Canvas write boundary, so late results cannot overwrite manual names or stale Question content. Reads never create a handle, spawn ACP, generate a title, or repair stored metadata; legacy unbound threads may derive a read-only fallback from their first persisted user prompt.
...
The [shared title contracts](../../packages/shared/src/types/api/conversation-title.ts) define `ConversationTitle { title, source }`, where source is `user | acp | generated | fallback | null`. `POST /api/agent/threads/titles/query` accepts `{ canvasId, threadIds }` (at most 100 IDs) and returns `{ titles: Record<string, ConversationTitle> }`, resolving a Question before thread metadata and returning an empty title when neither owner exists. `PUT /api/agent/threads/:threadId/title?canvasId=...` accepts a trimmed non-empty `{ title }` of at most 120 characters and returns the effective title; `404 thread_not_found` means no writable Question or durable thread exists. Pre-send unbound Chat manual names remain local until durable creation. Both endpoints are Canvas-scoped and validate all inputs. Frontend caches adopt exact backend projections rather than ranking sources or displaying raw ACP metadata. Relevant stream/session metadata events invalidate only the affected thread; asynchronous generation may require bounded follow-up queries. No new history surface or dedicated title SSE event is introduced; Question writes use existing Canvas Sync.

## 6. External agents (ACP)

[acp/](../../apps/server/src/modules/agent/acp) is the integration layer for external agents. Agentlet owns the single trusted harness catalogue and probes its own machine for GitHub Copilot, Claude Agent, Gemini, Codex, Qwen Code, Kimi Code CLI, OpenCode, Cursor, CodeBuddy, and Hermes Agent. Huabu automatically provisions ordinary persisted Profiles from discovery; the manual editor reads the same agentlet-backed catalogue and retains Custom command. Catalogue entries with official argument-based full-auto modes expose the existing manual auto-approve toggle; automatic Profiles use the ordinary ACP launch recipe without adding approval flags. See [Agent Profiles](./agent-profiles.md) for ownership, default workspaces, deduplication and retirement of manifest execution.
...
- [`@agenetes/agent-profile`](../../external/agenetes/packages/agent-profile) owns ordinary Profile schemas, CRUD and persistence, without Team discovery, setup or Config dependencies. `buildAcpWorkloadSpec()` reads the selected Profile and snapshots its explicit placement, command and working directory before Agenetes creation. Retired manifest Profiles are not selectable or compiled into executable recipes.
...
- [profile-store.ts](../../apps/server/src/modules/agent/acp/profile-store.ts) is a read-only source for one-time import of older command Profiles. The new `agent-profiles/registry.json` becomes authoritative after initialization; old Team data is left untouched and is not executed. Ordinary legacy command Profiles preserve their IDs and use the Huabu server working directory when an old record omitted `cwd`. [profiles.route.ts](../../apps/server/src/modules/agent/acp/profiles.route.ts) is the owner-only HTTP adapter over the generic registry, [spawn-orchestrator.ts](../../external/agenetes/packages/acp-driver/src/spawn-orchestrator.ts) targets the selected daemon, and [daemon.route.ts](../../apps/server/src/modules/agent/acp/daemon.route.ts) exposes supervised-daemon status and restart controls.
...
| File                                                                                                   | Responsibility                                                                                                    |
| ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- |
| [agent-thread.service.ts](../../apps/server/src/modules/agent/agent-thread.service.ts)                 | Admitted Preparing / Executing / Stopping / Settled lifetime, cancellation, outcome precedence, and lease cleanup |
| [agent-node-binding.ts](../../apps/server/src/modules/agent/agent-node-binding.ts)                     | Canonical record confirmation and Editing / Bound preparation guard                                               |
| [agent-node-lifecycle.ts](../../apps/server/src/modules/agent/agent-node-lifecycle.ts)                 | Fresh-state first-content and token-guarded terminal projection                                                   |
| [agent.service.ts](../../apps/server/src/modules/agent/agent.service.ts)                               | Internal lazy create, awaited binding confirmation, then controls/run                                             |
| [external-agent-realization.ts](../../apps/server/src/modules/agent/acp/external-agent-realization.ts) | External first-interaction single-flight and record-before-Bound-before-session ordering                          |
| [conversation-stores.ts](../../apps/server/src/modules/agent/agenetes/conversation-stores.ts)          | Portable existing Disk/SQLite conversation-store dispatch                                                         |
