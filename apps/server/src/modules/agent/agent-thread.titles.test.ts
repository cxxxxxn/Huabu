...
describe('Question ownership at the real message adapters', () => {
  for (const backend of ['internal', 'external'] as const) {
    it.each([
      ...
      `${backend}: %s uses unified title work before run`,
      async (owner) => {
        ...
        const service = new AgentThreadService({
          ...
          realizeExternal: async ({ requestedBinding, fixedTarget }) => {
            const externalBinding =
              fixedTarget?.agentBinding ?? requestedBinding;
        ...
        expect(conversationTitleService.start).toHaveBeenCalledTimes(1);
        {
          expect(conversationTitleService.start).toHaveBeenCalledWith(
            canvasId,
            threadId,
            'First prompt',
          );
          if (backend === 'internal') {
            expect(create.mock.invocationCallOrder[0]).toBeLessThan(
              vi.mocked(conversationTitleService.start).mock
