...
describe.each(
  ...
  it('preserves host title metadata through driver snapshots, restart, and rehome', async () => {
    ...
    await instance.updateHostMetadata(namespace, THREAD_ID, {
    ...
    const before = (await instance.record(namespace, THREAD_ID))!;
    ...
    expect(
      (await restarted.record(namespace, THREAD_ID))?.hostMetadata,
    ).toEqual(before.hostMetadata);
    ...
    expect(await restarted.record(namespace, THREAD_ID)).toBeUndefined();
    const moved = (await restarted.record(target, THREAD_ID))!;
  ...
  it('pages display-turn ranges without splitting null-request continuations', async () => {
    await openWithSpace();
    const namespace = canvasAcpNamespace(CANVAS_ID);
    const values = [
    ...
    const newest = await conversationTurnStore.page(namespace, THREAD_ID, {
      limit: 2,
    });
    expect(
    ...
    const older = await conversationTurnStore.page(namespace, THREAD_ID, {
      limit: 2,
  ...
  it('keeps a history cursor stable across append and rejects it after replace', async () => {
    await openWithSpace();
    const namespace = canvasAcpNamespace(CANVAS_ID);
    ...
    const cursor = (
      await conversationTurnStore.page(namespace, THREAD_ID, { limit: 1 })
    ).before!;
    await conversationTurnStore.append(namespace, THREAD_ID, {
      turn: {
    ...
    expect(
      (
        await conversationTurnStore.page(namespace, THREAD_ID, {
          limit: 1,
          before: cursor,
        })
    ...
    await conversationTurnStore.replace(namespace, THREAD_ID, []);
    // A synchronous store refuses the stale cursor by throwing, so the call
    // is made inside the thunk rather than handed over as a promise.
    await expect(async () =>
